...
- Canonical Receptionist v15 is the owner-approved condensed receptionist prompt. The legacy rendering injects tenant variables directly; the layered rendering keeps the generic rules byte-stable and appends tenant bindings and the persisted by-heart block in Business Details. With no pins, omit the by-heart section and every by-heart reference.
...
- Run `corepack pnpm validate:receptionist-v15`, `corepack pnpm validate:realtime2-payloads`, `corepack pnpm typecheck`, and `corepack pnpm build` before release.
...
- The synthetic Realtime battery requires explicit API-cost approval: `EVERYCALL_RUN_RECEPTIONIST_V15_REALTIME_ACCEPTANCE=1 corepack pnpm acceptance:receptionist-v15:realtime`. Optional comma-separated filters are `EVERYCALL_RECEPTIONIST_V15_ACCEPTANCE_MODES` and `EVERYCALL_RECEPTIONIST_V15_ACCEPTANCE_CASES`.
