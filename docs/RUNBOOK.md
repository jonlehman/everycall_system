...
## Deployments
- Latency-trial GPT-Live backend: `OPENAI_LIVE_BACKEND_MODEL=gpt-6-luna` and `OPENAI_LIVE_BACKEND_REASONING_EFFORT=none`; Fast mode remains off. The backend uses a prepared Responses WebSocket; both provider endpoints must be reachable. Verify model access, independent review, offline gates and authorized audible canary before release. Compare the new call's useful speech latency and judgment to the September 23 baseline; reduced reasoning is not acceptance by itself. Backend disconnect/cache loss permits one context-only retry; an unknown application action must be reconciled, not manually replayed. No database migration is needed.
...
## Logs
- Current Live-led flow: Luna consultation is asynchronous expert advice. Ordinary dialogue may continue before `backend_completed`, and optional advice should arrive as quiet context; a completed consultation is not proof the caller's goal was addressed. For the painting canary, correlate caller and assistant transcript order with the caller revision and assistant epoch, especially any late discarded advice, an open-goal recovery after "Hello?", and the next useful move. Only verified tenant facts and protected questions/actions should require an authoritative result. The separate audio-buffer/dropout work remains pending.
