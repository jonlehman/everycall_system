...
- Apply Part 9 with `EVERYCALL_APPLY_KNOWS_BY_HEART_MIGRATION=1 corepack pnpm migrate:knows-by-heart` before deploying routes that read the `kb_*` tables.
...
- Canonical Receptionist v18 retains the v17 flow and changes only the final close protocol: use the confirmed first name in `Thanks for calling, FIRSTNAME. Have a good one.` (or the exact no-name fallback), and never emit `finish_session` without the spoken closing in the same response.
...
- Run `corepack pnpm validate:receptionist-v18`, `corepack pnpm validate:realtime2-payloads`, `corepack pnpm typecheck`, and `corepack pnpm build` before release.
