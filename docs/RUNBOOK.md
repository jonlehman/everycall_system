...
- Canonical Receptionist v13 is the restored OpenAI v3 prompt plus the conditional by-heart accommodations, all reviewed v11/v12 rules, and the v13 first-name/spelled-surname protocol, one-beat negative example, and concision rules. With no pins, omit the by-heart section and every by-heart reference.
...
- Run `corepack pnpm validate:receptionist-v13`, `corepack pnpm validate:realtime2-payloads`, `corepack pnpm typecheck`, and `corepack pnpm build` before release.
...
- The synthetic Realtime battery requires explicit API-cost approval: `EVERYCALL_RUN_RECEPTIONIST_V13_REALTIME_ACCEPTANCE=1 corepack pnpm acceptance:receptionist-v13:realtime`. Optional comma-separated filters are `EVERYCALL_RECEPTIONIST_V13_ACCEPTANCE_MODES` and `EVERYCALL_RECEPTIONIST_V13_ACCEPTANCE_CASES`.
- The battery records assistant word counts across every turn; the combined average must be below 30. The capture case requires first-name echo, one surname spelling prompt, exact spelled capture, no later spoken surname, and no phone-number recap in the close.
- `data_capture` must be emitted without audio. Its continuation goes directly to the next needed question or to the first-name-plus-closing-phrase close; no status lead-in, wrap-up narration, or recap is allowed.
