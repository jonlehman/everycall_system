...
- Canonical Receptionist v16 retains the owner-approved v15 condensed receptionist prompt and adds smooth first-name handling plus the required other-questions/immediate-goodbye close. The legacy rendering injects tenant variables directly; the layered rendering keeps the generic rules byte-stable and appends tenant bindings and the persisted by-heart block in Business Details. With no pins, omit the by-heart section and every by-heart reference.
...
- Run `corepack pnpm validate:receptionist-v16`, `corepack pnpm validate:realtime2-payloads`, `corepack pnpm typecheck`, and `corepack pnpm build` before release.
...
- The synthetic Realtime battery requires explicit API-cost approval: `EVERYCALL_RUN_RECEPTIONIST_V16_REALTIME_ACCEPTANCE=1 corepack pnpm acceptance:receptionist-v16:realtime`. Optional comma-separated filters are `EVERYCALL_RECEPTIONIST_V16_ACCEPTANCE_MODES` and `EVERYCALL_RECEPTIONIST_V16_ACCEPTANCE_CASES`.
- The battery records assistant word counts across every turn; the combined average must be below 30. The capture case requires the first name at the start of the surname-spelling question with no preceding `Thanks` or adjacent punctuation, exact spelled capture, no later spoken surname, no name or phone-number recap in the close, the required other-questions checkpoint, and same-turn `finish_session` after the exact goodbye.
