...
- If substantive caller work has no delegation after 2 seconds of observed inbound quiet and at least 800 ms of transcript quiet, the gateway logs `delegation_missing` and sends one quiet instruction requesting delegation of the existing transcript (`delegation_requested`). Active caller speech, stopped inbound media, an existing backend task, or a delegation already settling suppresses this nudge. It does not invent a delegation ID or execute backend work; a real provider delegation is still required. An unanswered nudge remains visible in the trace for investigation rather than repeatedly asking the caller to speak.
