...
## Logs
- Conversation flow: `openai_live_conversation_decision` records backend beat, readiness, recommended move, hard boundary codes, question purpose and issued discovery count with request/delegation IDs. Terra now advises; Live chooses ordinary wording. `conversation_discovery_limit` means a third optional project question was withheld and sent through bounded handoff repair. `openai_live_lookup_decision` distinguishes purpose-bound knowledge work from an unexecuted call missing its intent. Neither event logs caller goal, missing fact text or private reasoning. Check actual lookup operation events before attributing a conversational pause to retrieval: a backend consultation can contain zero tool calls.
- `local_conversation_routed` records a safe courtesy or content-free clarification beat that did not call Terra. It never authorizes a question, fact, callback, contact capture, transfer or close. If Live misses delegation for a substantive statement, `delegation_missing` and `controller_fallback_started` must still appear. No local bypass should occur while a backend task, pending question, unresolved business question or uncertain action remains.
...
- For latency, group `openai_live_latency` by call SID and `requestId`: `caller_transcript_received` / `caller_speech_end_observed` → `caller_turn_finalized` → `delegation_received` → `backend_queued` / `backend_started` → `backend_request_sent` / `backend_first_event` / `backend_response_completed` → `handoff_validated` → `commentary_sent` → `live_audio_received` → `telnyx_audio_sent`. The historical `commentary_sent` milestone now carries `deliveryType=instructions` for consultation guidance; it does not imply Terra supplied a spoken sentence. Each backend exchange also has `exchangeId`, attempt and step; operations have `operation_started` / `operation_completed` with elapsed milliseconds. Fragment arrivals and failed handoffs retain the request ID and original start time. The first estimated speech end remains available even when the caller repeats or says hello again.
...
- If substantive caller work has no delegation after 2 seconds of observed inbound quiet and at least 800 ms of transcript quiet, the gateway logs `delegation_missing` and `controller_fallback_started`, finalizes the existing caller statement, and queues Terra through the same serialized backend path. Active caller speech, stopped inbound media, an existing backend task, or a delegation already settling suppresses this fallback. Work started by the application uses `delegationId:null`; it never invents a provider ID. A late provider delegation for the same revision logs `delegation_coalesced` and adopts the existing task without replaying speech or operations. Task audit includes `source=client_delegation|application_quiet_fallback`.
- The backend no longer supplies a spoken sentence. Invalid consultation shape, unsupported answer without verified facts, or an unauthorized question receives one bounded repair; a second failure uses the neutral inability-to-confirm message. A listen beat sends explicit guidance for Live to make a brief natural recognition. Confirm actual audible behavior in a caller canary; a caller statement does not have to contain a question to reach Terra.
