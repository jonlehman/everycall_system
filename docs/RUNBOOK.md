...
## Logs
- Conversation flow: `openai_live_conversation_decision` records backend beat, readiness, question purpose and issued discovery count with request/delegation IDs. `conversation_discovery_limit` means a third optional project question was withheld and sent through bounded handoff repair. `openai_live_lookup_decision` distinguishes purpose-bound knowledge work from an unexecuted call missing its intent. Neither event logs caller goal, missing fact text or private reasoning. Check actual lookup operation events before attributing a conversational pause to retrieval: a backend handoff can contain zero tool calls.
...
- `empty_caller_response` means the backend supplied no speech or question for a completed meaningful request. This is rejected and repaired once; a second failure speaks the existing neutral inability-to-confirm message. A listen beat must include an actual spoken reflection. A caller statement does not have to contain a question to reach the controller.
