...
- Run `corepack pnpm validate:receptionist-v17`, `corepack pnpm validate:realtime2-payloads`, `corepack pnpm typecheck`, and `corepack pnpm build` before release.
...
- The synthetic Realtime battery requires explicit API-cost approval: `EVERYCALL_RUN_RECEPTIONIST_V17_REALTIME_ACCEPTANCE=1 corepack pnpm acceptance:receptionist-v17:realtime`. Optional comma-separated filters are `EVERYCALL_RECEPTIONIST_V17_ACCEPTANCE_MODES` and `EVERYCALL_RECEPTIONIST_V17_ACCEPTANCE_CASES`.
