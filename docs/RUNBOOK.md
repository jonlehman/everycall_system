...
- Canonical Receptionist v14 is the restored OpenAI v3 prompt plus the conditional by-heart accommodations, all reviewed v11-v13 rules, and silent `knowledge_lookup` execution followed by a direct answer. With no pins, omit the by-heart section and every by-heart reference.
...
- Run `corepack pnpm validate:receptionist-v14`, `corepack pnpm validate:realtime2-payloads`, `corepack pnpm typecheck`, and `corepack pnpm build` before release.
...
- The synthetic Realtime battery requires explicit API-cost approval: `EVERYCALL_RUN_RECEPTIONIST_V14_REALTIME_ACCEPTANCE=1 corepack pnpm acceptance:receptionist-v14:realtime`. Optional comma-separated filters are `EVERYCALL_RECEPTIONIST_V14_ACCEPTANCE_MODES` and `EVERYCALL_RECEPTIONIST_V14_ACCEPTANCE_CASES`.
