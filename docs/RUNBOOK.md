...
## Deployments
- Managed GPT-Live backend: `OPENAI_LIVE_BACKEND_MODEL=gpt-6-luna` and `OPENAI_LIVE_BACKEND_REASONING_EFFORT=none`; Fast mode remains off. GPT-Live now owns the Responses backend connection and conversation context. The gateway still runs private function tools and confirms their current authorization. Verify model access, independent review, `node scripts/validate-managed-live-runtime.mjs`, offline gates and an authorized audible Telnyx canary before release. Compare useful speech latency and judgment to the September 23 baseline; a model-speed gain is not acceptance by itself. Unknown application actions must be reconciled, never blindly replayed. No database migration is needed.
