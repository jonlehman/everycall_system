...
4. `call-gateway` receives EveryCall prompt payload (system prompt, tenant greeting, tenant knowledge, field schema, tool definitions, session config).
   The prompt service loads the materialized `kb_block` for the active composite knowledge build; it does no ranking, rewriting, reconciliation, or AI generation on the call path.
