...
- Auto‑configure defaults (industry prompts, knowledge entries, guardrail questions, routing).
...
- Fully autonomous publishing of generated knowledge or guardrail answers without tenant review
...
2. **AI Enrichment (Draft)**
   - Load default industry knowledge entries and guardrail questions first.
   - Fetch website content and available Google Business Profile data.
   - Generate knowledge drafts and guardrail answers only when explicit supporting statements are found in source data.
   - Leave high-risk answers blank when explicit evidence is not found.
...
4. **Submit**
   ...
   - Seed industry prompts/knowledge defaults and default agent config.
   - Persist generated knowledge entries and guardrail answers only after tenant confirmation.
...
2. Generated knowledge drafts and guardrail answers must be reviewable/editable before publish.
3. Guardrail answer generation requires explicit source evidence; otherwise the answer remains blank.
4. Prefer source order: official website, then Google Business Profile data.
5. Store source attribution per generated knowledge draft or guardrail answer (`sourceType`, `sourceUrl`, `retrievedAt`) for auditability.
