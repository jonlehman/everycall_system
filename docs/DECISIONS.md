...
- Treat the Realtime model's `finish_session` tool call as authoritative. The gateway records and accepts every valid invocation and drains queued audio before hanging up; it no longer rejects the tool based on transcript-derived closing order. Keep the conversational close sequence in the canonical prompt and acceptance battery rather than duplicating it as a gateway state machine.
