...
- VAD: use xAI-native `server_vad` with a `0.9` activation threshold and a 200 ms silence endpoint. The higher activation threshold reduces speakerphone echo triggers while the shorter endpoint reduces the pause after caller speech. xAI owns automatic response creation and provider-side interruption; do not send OpenAI-only `eagerness`, `create_response`, or `interrupt_response` settings.
