...
## Managed GPT-Live/Responses delegation acceptance

- Build and run `node scripts/validate-managed-live-runtime.mjs`. Verify a Responses delegation at session start, completed function-item collection from nested `response.event`, one result per function call through `response.item.create`, and explicit `response.create` continuation. The gateway must reject stale, duplicate, parallel, or unauthorized effects without executing them.
- Exercise the 16:18 Pacific failure path: exterior painting request, callback offer, caller yes, contact capture and next step. Sarah must not enter a recurring `application_quiet_fallback` or `advice_discarded` loop, apologize for an unrelated unconfirmed action, or greet the same active call twice. Check that protected questions are prepared, spoken exactly, and answered before they authorize contact, transfer or close.
- On an authorized audible Telnyx canary, test a direct company-fact question, spelling and phone readback, refusal/correction, transfer consent, unsupported price/scheduling request, interruption during a pending tool, and a normal close. Confirm the transcript, tool ledger, useful first audio, and caller hearing independently. Audio buffering remains a separate acceptance concern.

## Historical client-delegation conversation control (rollback tests)
- Current release blocker: Live owns the conversation while Luna advises asynchronously. Withhold or delay Luna's optional advice and verify Sarah still makes useful progress on an ordinary painting request; she must not stall after "Let's see what we can do," wait for advice to choose every next question, or treat "Hello?" as a fresh greeting while the original goal remains open. Late advice from an older caller or assistant turn must not redirect the current dialogue. A failed optional consultation must not produce the old "I couldn't confirm that" apology; a requested database fact must still wait for verified information or receive a truthful inability-to-confirm answer. Assert that optional advice is delivered quietly rather than with an interrupting instruction or forced response.
- On the caller-run canary, record the spoken exchange and time from each caller turn to Sarah's useful next move. Acceptance requires a coherent path from the painting request to an allowed next step, direct questions answered before intake resumes, one question at a time, correction/refusal retained, and no unsupported price, scheduling, callback, transfer or completed-action claim. Check the transcript together with request and advice-lag logs; a WebSocket write confirms transmission, not hearing. Audio dropouts are tracked separately and are not addressed by this conversation-flow change.
- Live-led regression: greeting/thanks and an isolated content-free "Can you help me?" may be answered locally. A painting request, corrections and pending-question answers may progress conversationally while Luna consults in the background; the quiet fallback still obtains advice if the provider omits delegation. A local question never binds callback or transfer consent, and Live cannot turn remembered unverified business information into a claim. Compare useful first audio, coherent next steps, asynchronous backend counts and protected-action outcomes, not just a quick filler response.
- Collaborative-EQ canary (caller-run, separate calls): (1) Start silent; Sarah should greet unprompted. Say only "My house needs to be painted." Expect a natural recognition and useful next move, not a demand for a formal question or an unsupported capability claim. (2) Describe a project, ask a direct business question mid-story, then correct an earlier detail. Expect the answer first, correction retained, and no reset to a rigid questionnaire. (3) Say you are unsure about a callback, then decline it; no contact capture or repeated callback pressure. (4) Ask for an appointment or price without authorized facts; expect an honest limit, not a scheduling or price claim. (5) Interrupt Sarah during a reply, ask for a person, and answer yes only after the exact transfer confirmation; no premature transfer. Record Sarah's actual wording and awkward pauses, not merely tool success.
- Regression for the September 22 stall: say "My house needs to be painted" and stop. Offline tests withhold every Live delegation and verify the quiet fallback reaches Luna even if Live has replied; ordinary accepted/no-question advice stays quiet while an open goal remains. A late delegation during lookup or after completion must not duplicate tools or speech. Direct database questions still require authority; active speech, absent inbound media and ordinary backchannels do not start duplicate work. On the audible canary Sarah must meaningfully continue the original statement without requiring "Hello" or a rephrased question.
- Interruption regression: while a prepared response or capture waits, caller audio resumes, then produces one silent frame before its correction transcript arrives. The full sustained-quiet grace must protect both speech and operation dispatch; the delayed correction supersedes the old response without an old capture. Offline fixtures scale the production 800 ms grace to 80 ms.
- Adversarial gates: a third peeling/repair question labeled clarification or required_contact is rejected; contact exemptions require bound callback consent, a real missing field and its approved question. A fabricated caller quote or old turn cannot authorize lookup, and service-fit retrieval is constructed as a business-capability question. Genuine peeling-paint capability questions remain valid. "Okay"/"go on" after completed reflection resumes the controller without granting consent; an active-lookup backchannel does not. Natural callback yes forms and later "don't call me" cancellation are covered.
- Run gateway build and `node scripts/validate-live-runtime.mjs`. Its offline controller regressions cover the house-painting / exterior / whole-house call, observed discovery-question budget and invalid optional recommendations, autonomous callback opt-in, explicit callback interest, direct questions during discovery, missing lookup purpose, verified answers, hesitation, refusal, correction and preserved transfer/capture/closing safeguards. Scripted model replies prove the gates and state handling, not actual model judgment or naturalness.
- Audible acceptance: start with silence for Sarah's greeting, then describe a whole exterior repaint in your own words. Expect recognition, at most one or two useful discovery questions, and a natural callback transition when ready. On a separate call interrupt discovery with a business question, decline a callback, correct the scope, then ask about next steps. Sarah must answer first, preserve the correction/refusal, and avoid restarting discovery or claiming an appointment. Review the transcript alongside `openai_live_conversation_decision` and `openai_live_lookup_decision` and the existing latency trace.

## GPT-Live Backend Ownership
- Build the gateway, then run `node scripts/validate-live-runtime.mjs` and `node scripts/validate-audio-pump.mjs`. The Live suite is offline: fake sockets verify warmup, continuation, cache-loss recovery, abort and storage-disabled `none` reasoning; scripted backend fixtures verify facts, no scheduling/callback behavior, long/spelled input, corrections, lookup backchannels, precise transfer consent, unknown outcomes, failures, interrupted closing and noise/disconnect handling.
...
- The offline Live suite also verifies greeting without caller speech, matching/duplicate acknowledgements, continued inbound silence, startup timeout, caller speech beyond the greeting timeout, suppression of late greeting during caller speech, controller fallback when delegation is missing, fragment accumulation across quiet gaps, bounded handoff repair and neutral double-failure fallback, tool prohibition during repair, stable latency request IDs, mixed-frame byte provenance, and successful versus failed Telnyx write callbacks.
- An explicitly authorized real Telnyx canary must start with the caller silent and verify Sarah begins once. Repeat the varied matrix, including callback refusal, unsupported scheduling, price anchors, correction after capture, yes to an unrelated question, silence/noise and an interrupted close. Compare `openai_live_latency` request traces (`delegation_received`, backend exchange milestones, `handoff_validated`, `commentary_sent`, `live_audio_received`, `telnyx_audio_sent`) with audible behavior. A write callback confirms gateway transmission, not caller hearing; audio-to-delegation attribution is a temporal candidate. Verify `openai_live_backend_prepared` reports the selected backend model, `none`, `websocket`, and `store:false` before delegated work.
...
- Run `corepack pnpm validate:receptionist-v19` after changing pricing artifacts, lookup assembly, packet provenance, tenant price authorization, Section 02 authoring/notices, or the v19 prompt. The release-gating fixture remains the All Covered Painting packet containing both the exterior range and Painter-for-a-Day rates under different catalog categories; neither monetary expression may reach the exact model-facing tool output.
- Before live rollout, replay the zero-state pricing query through both API preview and call gateway, verify a tenant-authorized price survives lookup, verify a confirmed-but-unapproved monetary fact is blocked, and exercise caller/competitor price anchors without repetition or evaluation. Semantic source-verdict recall and known-negative precision require the labeled production fixture set; deterministic tests alone do not certify those model judgments.
...
- The Part 9 validator executes migrations 0046–0048 in PGlite, exercises protected write shapes with SQLSTATE `P9K01` plus price-authorization guard `P9K03`, verifies purge context and candidate immutability, checks deterministic serialization, and tests both sides of the contextual prompt-injection gate.
...
- Run `corepack pnpm validate:receptionist-v18` for the v17 flow with the current exact checkpoint `Is there anything else I can help you with?`, plus the first-name close, the prohibition on a function-call-only `finish_session` response, response-ID-keyed audio verification, and the one-shot audio-only gateway recovery with tools disabled.
- New-tenant prompt initialization must accept a business name with no sentence punctuation while leaving `company_description` and `basic_no_tool_allowed_statement` empty. Legacy rendering must omit the empty permission line; layered rendering must omit both empty Business Details bindings while keeping Layer 1 byte-identical to populated tenants. Publishing a website or setup-interview build remains responsible for storing the completed punctuated snapshot.
...
- Before pilot traffic, make one controlled live provider call and verify:
  ...
  - `Start Receptionist` mutes the operator, waits one second, and only then requests that the AI join; `End Receptionist` removes only the AI and unmutes the operator, and the manual mute control works in both states
