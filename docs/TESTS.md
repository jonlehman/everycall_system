...
- Selective-delegation regression: greeting/thanks and an isolated content-free "Can you help me?" may be answered locally without a Terra request or a watchdog retry. Mixed or foreign-language requests, "My house needs painting," a later "okay/go on" after a reflection, answers to pending questions, corrections and refusal must still delegate (or reach the quiet fallback). A local clarification never binds callback or transfer consent; no previous business fact is reused locally merely because Live remembers it. Compare backend call counts and useful first audio, not just a quick filler response.
...
- Regression for the September 22 stall: say "My house needs to be painted" and stop. Offline tests withhold every Live delegation and verify the quiet fallback reaches Terra; an empty listen result gets one repair and a spoken response. A late delegation during lookup or after completion must not duplicate tools or speech. Direct questions also use the fallback; active speech, absent inbound media and ordinary backchannels do not trigger it. Repeated empty replies yield an audible neutral failure, never a silent accepted handoff. On the audible canary Sarah must respond to the original statement without requiring "Hello" or a rephrased question.
...
- Run gateway build and `node scripts/validate-live-runtime.mjs`. Its offline controller regressions cover the latest house-painting / exterior / whole-house call; a third discovery question must be rejected before Live hears it, and a repair may specifically reflect and listen without a forced callback. They also cover explicit callback interest, direct questions during discovery, missing lookup purpose, answering from a known result, hesitation, refusal, correction and preserved transfer/capture/closing safeguards. Scripted model replies prove the gates and state handling, not actual model judgment or naturalness.
