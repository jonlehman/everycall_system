...
import { assignSupportConversation } from "../../../../_lib/supportChat.js";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const clearAssignment = body.clear === true;
    const adminUserId = clearAssignment ? null : Number(body.adminUserId || admin.id);
    const tenantKey = await assignSupportConversation(pool, {
      conversationId,
      adminUserId
    });
    await writeAuditLog(pool, {
      ...
      details: {
        conversationId,
        assignedAdminUserId: clearAssignment ? null : adminUserId
      }
    });
    return res.status(200).json({ ok: true });
  } catch (err) {
    return res.status(err?.statusCode || 500).json({
      error: "admin_support_assign_error",
