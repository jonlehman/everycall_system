...
import { listKnowledgeOverrides, saveKnowledgeOverride } from "../../_lib/knowledgeReceptionistConfig.js";
...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === "GET") {
      const overrides = await listKnowledgeOverrides(pool, tenantKey);
      return res.status(200).json({ ok: true, overrides });
    }

    if (req.method === "POST") {
      const manager = await requireTenantRoles(res, session, ["owner", "admin"], {
        message: "Only account admins and owners can update knowledge overrides."
      });
      if (!manager) return;
      const body = typeof req.body === "object" && req.body ? req.body : {};
      const override = await saveKnowledgeOverride(pool, tenantKey, body.override || body, session);
    ...
    return fail(res, 500, "knowledge_override_error", message);
  }
}
