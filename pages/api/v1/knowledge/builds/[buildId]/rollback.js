...
export default async function handler(req, res) {
  ...
  try {
    ...
    const result = await rollbackKnowledgeBuild(pool, tenantKey, buildId);
    ...
    if (message === "no_active_build") {
      return fail(res, 409, "no_active_build", "There is no active build to roll back.");
    }
    if (message === "build_already_active") {
      return fail(res, 409, "build_already_active", "The requested build is already active.");
    }
    return fail(res, 500, "build_rollback_error", message);
  }
}
