...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/v1/overview')
      ...
      .then((data) => {
        ...
        setStats(data.stats || { callsToday: 0, missed: 0, urgent: 0, callbacksDue: 0 });
  ...
  const status = loadState === 'error'
    ? { tone: 'bad', message: 'Could not load dashboard data. Refresh your browser to retry.' }
    : loadState === 'loading'
      ? { tone: 'warn', message: 'Loading today’s dashboard...' }
      : stats.urgent > 0 || stats.callbacksDue > 0
        ...
        : { tone: 'ok', message: 'Queue is clear. No urgent follow-up is waiting right now.' };

  return (
    <ClientPage
      title="Dashboard"
      subtitle="See today’s workload, outstanding follow-up, and recent activity."
      status={status}
      primaryAction={{ href: '/client/calls', label: 'Open Calls', brand: true }}
    >
      <div className="flex flex-wrap gap-2">
        <Link className={cn(buttonVariants({ variant: 'outline' }))} href="/client/receptionist/go-live">Open Go Live</Link>
        <Link className={cn(buttonVariants({ variant: 'outline' }))} href="/client/team">Open Team</Link>
      </div>
      ...
      <div className="mt-3 grid grid-cols-1 gap-3 xl:grid-cols-[1.2fr_.8fr]">
        ...
        <div className="rounded-xl border border-border bg-card p-3 shadow-sm">
          <h2 className="mt-0 text-lg font-semibold">Needs Attention</h2>
          <p className="text-sm text-slate-500">Calls that likely need callback or dispatch follow-up.</p>
          <div className="grid grid-cols-1 gap-2 text-sm md:grid-cols-[180px_1fr]">
            {loadState === 'error' ? (
              <div className="text-slate-500">Unable to load follow-up queue.</div>
            ) : actionQueue.length === 0 ? (
              <div className="text-slate-500">No follow-up items are pending.</div>
