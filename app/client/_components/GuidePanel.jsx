...
export default function GuidePanel({ title, eyebrow = 'Guide', icon = 'architecture', className = '', children }) {
  return (
    ...
    >
      ...
      {eyebrow ? (
        <div className="text-[11px] font-extrabold normal-case tracking-normal text-slate-500">{eyebrow}</div>
      ) : null}
      <div className={`${eyebrow ? 'mt-4' : ''} grid gap-4 text-sm leading-6 text-slate-600`}>{children}</div>
    </aside>
  );
}
