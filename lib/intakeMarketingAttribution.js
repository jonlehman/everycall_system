...
function buildMarketingAttribution(values = {}, extraCandidates = []) {
  ...
  const billingCycle = normalizeString(values.billingCycle);
  const utm = {};
  const clickIds = {};
  ...
  if (billingCycle) result.billingCycle = billingCycle;
...
function normalizeMarketingAttributionFromSearchParams(searchParams) {
  ...
  for (const [rawKey, rawValue] of searchParams.entries()) {
    ...
    else if (lowerKey === "billing_cycle") known.billingCycle = rawValue;
  ...
  return buildMarketingAttribution(
    {
      ...
      billingCycle: pickFirstMeaningful([source.billingCycle, source.billing_cycle]),
